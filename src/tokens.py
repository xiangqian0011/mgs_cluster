...
class JiebaTokenizer:
    def __init__(self, stop_words_path, user_dict=None, mode='s'):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 6 or sys.argv[1] not in ['-s', '-m'] or sys.argv[4] not in ['c', 's']:
        ...
        exit(-1)
    file_mode, input_filepath, output_filepath, cut_mode, stopword_file, user_dict = sys.argv[1:]
    jt = JiebaTokenizer(stopword_file, user_dict, cut_mode)
