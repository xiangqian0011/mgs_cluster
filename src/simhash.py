...
def hamming_distance(hash_a, hash_b, hashbits=gl.gl_HASHBIT):
...
class SimhashBuilder:
    def __init__(self, word_list=[], hashbits=gl.gl_HASHBIT):
        self.hashbits = hashbits
        self.hashval_list = [self._string_hash(word) for word in word_list]
        print 'SimhashBuilder Totally: %s words' %(len(self.hashval_list),)
...
class simhash():
    def __init__(self, tokens='', hashbits=gl.gl_HASHBIT):
        self.hashbits = hashbits
    ...
    def simhash(self, tokens):
        ...
        for t in [self._string_hash(x) for x in tokens]:
            ...
            for i in range(self.hashbits):
                ...
                if t & bitmask:
                    finger[i] += 1 #查看当前bit位是否为1，是的话则将该位+1
                else:
                    finger[i] += -1 #否则得话，该位减1
